...
class RectangularRoom(object):
    ...
    def clean_tile_at_position(self, pos, capacity):
        ...
        self.tiles_dirt[(x_cord, y_cord)] = self.tiles_dirt[(x_cord, y_cord)] - capacity
        if self.tiles_dirt[(x_cord, y_cord)] < 0 :
            self.tiles_dirt[(x_cord, y_cord)] = 0
        else :
            self.tiles_dirt[(x_cord, y_cord)] = self.tiles_dirt[(x_cord, y_cord)]
    ...
    def get_num_cleaned_tiles(self):
        ...
        """
        clean_tiles = 0
        for pos in self.tiles_dirt.keys() :
            if RectangularRoom.is_tile_cleaned(self, pos[0], pos[1]) == True :
                clean_tiles += 1
        return clean_tiles
...
class FurnishedRoom(RectangularRoom):
    ...
    def is_position_furnished(self, pos):
        ...
        """
        return self.is_tile_furnished(math.floor(pos.x), math.floor(pos.y))
...
#        Robot.__init__(self, room, speed, capacity)
    ...
    def update_position_and_clean(self):
        ...
        """
        direction_cur = self.get_robot_direction()
        position_cur= self.get_robot_position()
        speed_cur = self.speed
        position_new = position_cur.get_new_position(direction_cur, speed_cur)
        if self.room.is_position_valid(position_new):
            self.set_robot_position(position_new)
            RectangularRoom.clean_tile_at_position(self.room, self.position, self.capacity)
        else:
...
class FaultyRobot(Robot):
    ...
    def update_position_and_clean(self):
        ...
        speed_cur = self.speed
        position_new = position_cur.get_new_position(direction_cur, speed_cur)
        if self.room.is_position_valid(position_new) and not self.gets_faulty():
            self.set_robot_position(position_new)
            RectangularRoom.clean_tile_at_position(self.room, self.position, self.capacity)
        else:
            self.set_robot_direction(random.random()*360)         
...
def run_simulation(num_robots, speed, capacity, width, height, dirt_amount, min_coverage, num_trials,
    ...
    robot_type: class of robot to be instantiated (e.g. StandardRobot or
                FaultyRobot)
    """
    trial_counter,time_step = 0,0
    
    trial_dict = {}

    while trial_counter < num_trials:
        room = EmptyRoom(width, height, dirt_amount)
#            If you dont want the visualization comment out the animation parts and the partts concerning the type of room.
#        if type(room) == EmptyRoom:
#            is_furnished = False
#        else :
#            is_furnished = True

#        anim = ps3_visualize.RobotVisualization(num_robots, width, height, is_furnished, delay = .01)           
        robot_dict = {}
        for i in range(num_robots):
            robot_dict[i] = robot_type(room, speed, capacity)
#            robots cleaning each have a number/therefore a name.
        tiles_in_room = room.get_num_tiles()
        time_step +=1
        while min_coverage > room.get_num_cleaned_tiles() / tiles_in_room :
            time_step += 1
            for robot in robot_dict.values() :
                robot.update_position_and_clean()
#            anim.update(room, robot_dict.values())
        trial_dict[trial_counter] = time_step 
        time_step = 0        
        trial_counter += 1
#        anim.done()

        
    return sum(trial_dict.values()) / num_trials
...
#       of a 20x20 room?
#   For the two robot types it always take the faulty robot to clean longer than the standard robot but as there are more and more robots the difference becomes inconsequential.
#   With 2 robots std bot is noticably quicker than the fty bot.
#
# 2) How does the performance of the two robot types compare when two of each
#       robot cleans 80% of rooms with dimensions 
#       10x30, 20x15, 25x12, and 50x6?
#       There is a dip in the time when the aspect ratio becomes 2 from <= 1,but afterward it increases as the aspect ratio becomes greater. The rate of change is greater for
#       the Faulty bot.
#
